...
export function HistoryView() {
  ...
  // page or page size changes -- cached per-uuid/page/page-size so switching
  // back to an already-viewed search+page+size doesn't refetch.
  useEffect(() => {
    if (!selectedSearch || selectedSearch.status !== "COMPLETE") return;
    const cacheKey = `${selectedSearch.uuid}:${resultsPage}:${resultsPageSize}`;
    if (resultsCache[cacheKey]) return;
    let cancelled = false;
    setResultsLoading(true);
    getArchiveSearchResults(selectedSearch.uuid, resultsPage, resultsPageSize)
      .then((page) => {
  ...
  }, [selectedSearch?.uuid, selectedSearch?.status, resultsPage, resultsPageSize]);
  ...
  // what the user happens to have the results table set to.
  async function handleDownloadCsv(search: ArchiveSearchSummary) {
    setDownloadingCsv(true);
    try {
      const firstPageKey = `${search.uuid}:1:${DEFAULT_PAGE_SIZE}`;
      const firstPage =
      ...
      for (let p = 2; p <= totalPages; p++) {
        const cached = resultsCache[`${search.uuid}:${p}:${DEFAULT_PAGE_SIZE}`];
  ...
  const resultsForSelected = selectedSearch
    ? (resultsCache[`${selectedSearch.uuid}:${resultsPage}:${resultsPageSize}`] ?? null)
    : null;
