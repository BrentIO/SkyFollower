...
class TestAssertUrlReachable:
    def test_200_passes(self):
        with patch("shared.url_reachability.requests.head", return_value=_mock_response(200)):
            assert_url_reachable("https://example.com", "example-runner")

    def test_2xx_passes(self):
        with patch("shared.url_reachability.requests.head", return_value=_mock_response(204)):
            assert_url_reachable("https://example.com", "example-runner")

    def test_404_fails(self):
        with patch("shared.url_reachability.requests.head", return_value=_mock_response(404)):
            with pytest.raises(AssertionError, match="404"):
                assert_url_reachable("https://example.com", "example-runner")

    def test_500_fails(self):
        with patch("shared.url_reachability.requests.head", return_value=_mock_response(500)):
            with pytest.raises(AssertionError, match="500"):
                assert_url_reachable("https://example.com", "example-runner")

    def test_405_is_hard_failure_no_get_fallback(self):
        with patch("shared.url_reachability.requests.head", return_value=_mock_response(405)) as mock_head:
            with pytest.raises(AssertionError, match="405"):
                assert_url_reachable("https://example.com", "example-runner")
        mock_head.assert_called_once()

    def test_failure_message_includes_runner_name_and_url(self):
        with patch("shared.url_reachability.requests.head", return_value=_mock_response(503)):
            with pytest.raises(AssertionError, match="example-runner.*https://example.com.*503"):
                assert_url_reachable("https://example.com", "example-runner")

    def test_request_exception_fails(self):
        with patch("shared.url_reachability.requests.head", side_effect=requests.ConnectionError("refused")):
            with pytest.raises(AssertionError, match="example-runner"):
                assert_url_reachable("https://example.com", "example-runner")

    def test_timeout_exception_fails(self):
        with patch("shared.url_reachability.requests.head", side_effect=requests.Timeout("timed out")):
            with pytest.raises(AssertionError):
                assert_url_reachable("https://example.com", "example-runner")

    def test_passes_headers_and_verify_through(self):
        with patch("shared.url_reachability.requests.head", return_value=_mock_response(200)) as mock_head:
    ...
    def test_default_headers_none_and_verify_true(self):
        with patch("shared.url_reachability.requests.head", return_value=_mock_response(200)) as mock_head:
            assert_url_reachable("https://example.com", "example-runner")
        mock_head.assert_called_once_with(
            "https://example.com", headers=None, verify=True, timeout=10, allow_redirects=True,
        )
