...
logger = logging.getLogger("fr-dgac-registry")
...
def _type_check_passes(simple_record: dict, detail_model_str: str) -> bool:
    """Return True if the DGAC model string is consistent with the simple record's type info.

    Both simple and detail tokens must be non-empty for the check to be
    meaningful; if either side yields no tokens the check is skipped (passes).
    """
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        ...
        name=f"SkyFollower {country_flag('FR')} France DGAC Registry Runner",
        model=f"{country_flag('FR')} France DGAC Registry Runner",
    )
    stats = [
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_fr_dgac_registry_{name}",
            "object_id": f"SkyFollower_runner_fr_dgac_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_fr_dgac_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
