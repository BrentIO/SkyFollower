...
logger = logging.getLogger("ky-caa-registry")

PDF_URL = "https://www.caacayman.com/wp-content/uploads/Active-Aircraft-Register.pdf"
REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/ky-caa-registry"
BATCH_SIZE = 100
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        identifier="SkyFollower_runner_ky_caa_registry",
        name=f"SkyFollower Cayman Islands {country_flag('KY')} CAA Registry Runner",
        model=f"Cayman Islands {country_flag('KY')} CAA Registry Runner",
        configuration_url="https://brentio.github.io/SkyFollower/runners/ky-caa-registry.html",
    )
    stats = [
        ("records_imported", "Cayman Islands CAA Registry Records Imported", "mdi:airplane", "total_increasing", None),
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_ky_caa_registry_{name}",
            "object_id": f"SkyFollower_runner_ky_caa_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_ky_caa_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
