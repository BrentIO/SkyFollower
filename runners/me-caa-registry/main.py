...
logger = logging.getLogger("me-caa-registry")
...
MQTT_ROOT = "SkyFollower/runner/me-caa-registry"
BATCH_SIZE = 100
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        identifier="SkyFollower_runner_me_caa_registry",
        name=f"SkyFollower Montenegro {country_flag('ME')} CAA Registry Runner",
        model=f"Montenegro {country_flag('ME')} CAA Registry Runner",
        configuration_url="https://brentio.github.io/SkyFollower/runners/me-caa-registry.html",
    )
    stats = [
        ("records_imported", "Montenegro CAA Registry Records Imported", "mdi:airplane", "total_increasing", None),
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_me_caa_registry_{name}",
            "object_id": f"SkyFollower_runner_me_caa_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_me_caa_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
