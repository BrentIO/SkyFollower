...
Published once, at the end of a run, to `SkyFollower/runner/au-casa-registry/statistic/{name}` (all retained):
...
Home Assistant autodiscovery configs are also published (retained) to `homeassistant/sensor/SkyFollower_runner_au_casa_registry_{name}/config` for each of the three stats above.
