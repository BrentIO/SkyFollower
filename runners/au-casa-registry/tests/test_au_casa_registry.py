...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "au_casa_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["au_casa_registry_main"] = mod
...
class TestApplyTypeLookup:
    ...
    def test_designator_resolves_sets_description_code(self):
        record = {"aircraft": {"type_designator": "EC25"}}
        r = self._make_redis({"manufacturer_model": "AIRBUS HELICOPTERS EC-225/725", "description_code": "H2T"})
        _apply_type_lookup(record, r)
        assert record["aircraft"]["description_code"] == "H2T"
...
class TestWriteToRedis:
    ...
    def test_source_field_in_written_record(self):
        ...
        write_to_redis(rows, r, REDIS_TTL)
        set_call = r.pipeline.return_value.json.return_value.set.call_args
        assert set_call[0][2]["source"] == "au-casa-registry"
...
class TestPublishCompletionStats:
    ...
    def test_mqtt_connect_timeout_does_not_raise(self):
        ...
        with patch("au_casa_registry_main.mqtt.Client") as mock_cls:
            mock_client = MagicMock()
    ...
    def test_mqtt_publishes_records_imported(self):
        ...
        with patch("au_casa_registry_main.mqtt.Client", return_value=mc):
    ...
    def test_mqtt_root_topic(self):
        assert MQTT_ROOT == "SkyFollower/runner/au-casa-registry"
