...
Published once, at the end of a run, to `SkyFollower/runner/bs-caa-registry/statistic/{name}` (all retained):
...
Home Assistant autodiscovery configs are also published (retained) to `homeassistant/sensor/SkyFollower_runner_bs_caa_registry_{name}/config` for each of the three stats above.
