#!/usr/bin/env python3
"""
SkyFollower Papua New Guinea CASA PNG Data Runner
...
CASA PNG index page — the first PDF link is always the most recent register.

No header row is present in the PDF tables; columns are identified by position.

Data source index: https://casapng.gov.pg/safety-regulatory/airworthiness/Aircraft-Registers/
"""
...
logger = logging.getLogger("pg-casapng-registry")

_INDEX_URL = "https://casapng.gov.pg/safety-regulatory/airworthiness/Aircraft-Registers/"
REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/pg-casapng-registry"
BATCH_SIZE = 100

# Column positions (no header row in PDF)
_COL_REGISTRATION = 0
_COL_MANUFACTURER = 1
_COL_MODEL = 2
_COL_OPERATOR = 3
...
def _discover_pdf_url(session: requests.Session) -> str:
    """Scrape the CASA PNG index page and return the href of the first PDF link."""
    logger.info("Fetching CASA PNG register index from %s", _INDEX_URL)
    ...
    raise RuntimeError("No PDF link found on CASA PNG index page.")


def download_and_parse(session: requests.Session) -> list[dict]:
    """Discover, download, and parse the Papua New Guinea CASA PNG aircraft register PDF."""
    pdf_url = _discover_pdf_url(session)
    logger.info("Downloading Papua New Guinea CASA PNG aircraft register from %s", pdf_url)
        ...
        for page in pdf.pages:
            ...
            for row in table:
                ...
                records.append({
                    ...
                    "operator": cells[_COL_OPERATOR] if len(cells) > _COL_OPERATOR else "",
                    "address": cells[_COL_ADDRESS] if len(cells) > _COL_ADDRESS else "",
                })

    logger.info("Parsed %d P2- rows from PDF.", len(records))
    return records
...
def write_to_redis(rows: list[dict], r: redis_lib.Redis, ttl: int) -> int:
    """Write CASA PNG data to aircraft:detail keys in Redis. Returns count written."""
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = {
        "ids": "SkyFollower_runner_pg_casapng_registry",
        "name": "SkyFollower Papua New Guinea CASA PNG Runner",
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_pg_casapng_registry_{name}",
            "object_id": f"SkyFollower_runner_pg_casapng_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_pg_casapng_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
...
def main() -> None:
    ...
    try:
        ...
        status = "success"
        logger.info(
            "Papua New Guinea CASA PNG runner completed successfully. Records imported: %d",
            records_imported,
        )

    except Exception as exc:
        logger.error("Papua New Guinea CASA PNG runner failed: %s", exc, exc_info=True)
