...
See `specs/data-dictionary.yaml` (`pg-casapng-registry` entry) for full column semantics and cross-source schema notes.
...
Published once, at the end of a run, to `SkyFollower/runner/pg-casapng-registry/statistic/{name}` (all retained):
...
Home Assistant autodiscovery configs are also published (retained) to `homeassistant/sensor/SkyFollower_runner_pg_casapng_registry_{name}/config` for each of the three stats above.
