...
logger = logging.getLogger("ge-gcaa-registry")

_PAGE_URL = "https://gcaa.ge/civil-aircraft-register/"

REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/ge-gcaa-registry"
BATCH_SIZE = 100
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        ...
        name=f"SkyFollower {country_flag('GE')} Georgia GCAA Registry Runner",
        model=f"{country_flag('GE')} Georgia GCAA Registry Runner",
    )
    stats = [
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_ge_gcaa_registry_{name}",
            "object_id": f"SkyFollower_runner_ge_gcaa_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_ge_gcaa_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
