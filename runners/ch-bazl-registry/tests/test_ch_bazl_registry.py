...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "ch_bazl_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["ch_bazl_registry_main"] = mod
...
class TestDownloadRegisterLogging:
    def test_logs_api_url(self):
        ...
        with patch("ch_bazl_registry_main.logger") as mock_logger:
            try:
...
class TestApplyTypeLookup:
    ...
    def test_designator_resolves_sets_manufacturer_model(self):
        record = {"aircraft": {"type_designator": "BU33"}}
        r = self._make_redis({"manufacturer_model": "BUCKER Bu-133 Jungmeister"})
        _apply_type_lookup(record, r)
        assert record["aircraft"]["manufacturer_model"] == "BUCKER Bu-133 Jungmeister"

    def test_designator_resolves_sets_description_code(self):
        record = {"aircraft": {"type_designator": "BU33"}}
        r = self._make_redis({"manufacturer_model": "BUCKER Bu-133 Jungmeister", "description_code": "L1P"})
        _apply_type_lookup(record, r)
...
class TestWriteToRedis:
    ...
    def test_manufacturer_model_set_from_type_lookup_end_to_end(self):
        ...
        write_to_redis([_make_row(type_designator="B77W")], r, REDIS_TTL)
        written = r.json.return_value.set.call_args.args[2]
        assert written["aircraft"]["manufacturer_model"] == "BOEING 777-300ER"
    ...
    def test_source_field_set(self):
        ...
        write_to_redis([_make_row()], r, REDIS_TTL)
        written = r.json.return_value.set.call_args.args[2]
        assert written["source"] == "ch-bazl-registry"
...
class TestMqttCompletionStats:
    ...
    def test_publishes_records_imported(self):
        ...
        with patch("ch_bazl_registry_main.mqtt.Client", return_value=mc):
