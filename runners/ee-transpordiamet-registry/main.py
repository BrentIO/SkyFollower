...
logger = logging.getLogger("ee-transpordiamet-registry")

_PAGE_URL = "https://transpordiamet.ee/ohusoidukite-register"

REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/ee-transpordiamet-registry"
BATCH_SIZE = 100
HEADER_ROWS = 2
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        ...
        name=f"SkyFollower {country_flag('EE')} Estonia Transpordiamet Registry Runner",
        model=f"{country_flag('EE')} Estonia Transpordiamet Registry Runner",
    )
    stats = [
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_ee_transpordiamet_registry_{name}",
            "object_id": f"SkyFollower_runner_ee_transpordiamet_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_ee_transpordiamet_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
