...
REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/no-caa-registry"
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        identifier="SkyFollower_runner_no_caa_registry",
        name=f"SkyFollower Norway {country_flag('NO')} CAA Registry Runner",
        model=f"Norway {country_flag('NO')} CAA Registry Runner",
        configuration_url="https://brentio.github.io/SkyFollower/runners/no-caa-registry.html",
    )
    stats = [
        ("records_imported", "Norway CAA Registry Records Imported", "mdi:airplane", "total_increasing", None),
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_no_caa_registry_{name}",
            "object_id": f"SkyFollower_runner_no_caa_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_no_caa_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
