...
def write_to_redis(conn: sqlite3.Connection, r: redis_lib.Redis, ttl: int) -> int:
    ...
    for row in rows:
        ...
        if acft_row is not None and row["icao_hex"] not in mictronics_designators:
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        ...
        name=f"SkyFollower US {country_flag('US')} FAA Registry Runner",
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_us_faa_registry_{name}",
            "object_id": f"SkyFollower_runner_us_faa_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_us_faa_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
