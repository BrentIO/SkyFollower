#!/usr/bin/env python3
"""
SkyFollower Moldova CAA Data Runner
...
PDF columns (no header row; positions are 0-based):
  ...
  2: Registration     (ER-prefix; used as lookup key)
  3: Serial No.       (stored as aircraft.serial_number)
  4: Operator*        (3-letter operator code only; not stored)
...
logger = logging.getLogger("md-caa-registry")

_PDF_URL = "https://www.caa.md/modules/filemanager/files/documentum/Registrul_Aerian_al_Republicii_Moldova.pdf"

REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/md-caa-registry"
BATCH_SIZE = 100
...
def download_and_parse(session: requests.Session) -> list[dict]:
    """Download and parse the Moldova CAA aircraft register PDF."""
    logger.info("Downloading Moldova CAA aircraft register from %s", _PDF_URL)
    resp = session.get(_PDF_URL, timeout=180)
    if not resp.ok:
    ...
    logger.info("Parsed %d ER- rows from PDF.", len(records))
    return records
...
def write_to_redis(rows: list[dict], r: redis_lib.Redis, ttl: int) -> int:
    """Write Moldova CAA data to aircraft:detail keys in Redis. Returns count written."""
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        identifier="SkyFollower_runner_md_caa_registry",
        name=f"SkyFollower Moldova {country_flag('MD')} CAA Registry Runner",
        model=f"Moldova {country_flag('MD')} CAA Registry Runner",
        configuration_url="https://brentio.github.io/SkyFollower/runners/md-caa-registry.html",
    )
    stats = [
        ("records_imported", "Moldova CAA Registry Records Imported", "mdi:airplane", "total_increasing", None),
        ("last_run_at", "Moldova CAA Last Run At", "mdi:clock", None, None),
        ("last_run_status", "Moldova CAA Last Run Status", "mdi:check-circle", None, None),
    ]
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            "state_topic": f"{MQTT_ROOT}/statistic/{name}",
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_md_caa_registry_{name}",
            "object_id": f"SkyFollower_runner_md_caa_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_md_caa_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
...
def main() -> None:
    ...
    try:
        ...
        status = "success"
        logger.info(
            "Moldova CAA runner completed successfully. Records imported: %d",
            records_imported,
        )

    except Exception as exc:
        logger.error("Moldova CAA runner failed: %s", exc, exc_info=True)
