#!/usr/bin/env python3
"""
SkyFollower Serbia CAD Data Runner
...
logger = logging.getLogger("rs-cad-registry")

_API_URL = (
    "https://apps.cad.gov.rs/ords/dcvws/regvaz/site/listAircraft"
    "?p_reg_id=0&p_type_ac=0&p_manufacturer=0&p_man_code=0&p_sn=0&p_user=0&limit=10000"
)
REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/rs-cad-registry"
BATCH_SIZE = 100
...
def download_and_parse(session: requests.Session) -> list[dict]:
    ...
    records = [
        ...
        if str(item.get(_FIELD_REGISTRATION, "")).startswith("YU-")
    ]
    logger.info("Parsed %d YU- rows from API response.", len(records))
    return records
...
def _build_record(row: dict, icao_hex: str, registration: str) -> dict:
    ...
    model = str(row.get(_FIELD_MODEL, "") or "").strip()
    if model:
        aircraft_fields["model"] = model

    serial = str(row.get(_FIELD_SERIAL, "") or "").strip()
    if serial:
        aircraft_fields["serial_number"] = serial

    operator = str(row.get(_FIELD_OPERATOR, "") or "").strip()
    if operator:
...
def write_to_redis(rows: list[dict], r: redis_lib.Redis, ttl: int) -> int:
    """Write Serbia CAD data to aircraft:detail keys in Redis. Returns count written."""
    reg_row_map: dict[str, dict] = {}
    for row in rows:
        reg = str(row.get(_FIELD_REGISTRATION, "") or "").strip()
        if not reg:
            continue
        reg_row_map[reg] = row
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = {
        "ids": "SkyFollower_runner_rs_cad_registry",
        "name": f"SkyFollower {country_flag('RS')} Serbia CAD Registry Runner",
        "manufacturer": "P5Software, LLC",
    }
    stats = [
        ("records_imported", "Serbia CAD Records Imported", "mdi:airplane", "total_increasing", None),
        ("last_run_at", "Serbia CAD Last Run At", "mdi:clock", None, None),
        ("last_run_status", "Serbia CAD Last Run Status", "mdi:check-circle", None, None),
    ]
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            "state_topic": f"{MQTT_ROOT}/statistic/{name}",
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_rs_cad_registry_{name}",
            "object_id": f"SkyFollower_runner_rs_cad_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_rs_cad_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
...
def main() -> None:
    ...
    try:
        ...
        status = "success"
        logger.info(
            "Serbia CAD runner completed successfully. Records imported: %d",
            records_imported,
        )

    except Exception as exc:
        logger.error("Serbia CAD runner failed: %s", exc, exc_info=True)
