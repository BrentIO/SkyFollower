#!/usr/bin/env python3
"""
SkyFollower Hungary KoZHAF Data Runner
...
page under the stable path /documents/66238/342548/ and must be discovered
fresh each run as the filename and query parameters change.

Registration marks in the PDF have a stray space after the hyphen
(e.g. "HA- GZQ") — normalized to "HA-GZQ" before lookup.

No table header row is reliable (bilingual Hungarian/English); columns are
identified by position.

Data source index: https://kozlekedesihatosag.kormany.hu/hu/dokumentum/104604
"""
...
logger = logging.getLogger("hu-kozhaf-registry")

_INDEX_URL = "https://kozlekedesihatosag.kormany.hu/hu/dokumentum/104604"
_PDF_PATH_FRAGMENT = "/documents/66238/342548/"
REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/hu-kozhaf-registry"
BATCH_SIZE = 100
...
# Column positions (bilingual header row present but not relied upon)
_COL_REGISTRATION = 0
_COL_MODEL = 1
_COL_SERIAL = 2
_COL_YEAR = 3
_COL_OWNER_NAME = 4
_COL_OWNER_ADDRESS = 5
_COL_OPERATOR_NAME = 6

_WHITESPACE_RE = re.compile(r"\s+")
_HA_SPACE_RE = re.compile(r"^HA-\s+")
...
def _normalize_registration(raw: str) -> str:
    """Normalize 'HA- GZQ' → 'HA-GZQ' by removing space(s) after the hyphen."""
    return _HA_SPACE_RE.sub("HA-", raw.strip())
...
def _discover_pdf_url(session: requests.Session) -> str:
    """Scrape the KoZHAF index page and return the PDF download URL."""
    logger.info("Fetching Hungary KoZHAF register index from %s", _INDEX_URL)
    ...
    for a in soup.find_all("a", href=True):
        href = a["href"]
        if _PDF_PATH_FRAGMENT in href and "download=true" in href:
            if not href.startswith("http"):
                from urllib.parse import urljoin
                href = urljoin(_INDEX_URL, href)
            return href

    raise RuntimeError(f"No PDF download link found on KoZHAF index page (expected path fragment: {_PDF_PATH_FRAGMENT}).")


def download_and_parse(session: requests.Session) -> list[dict]:
    """Discover, download, and parse the Hungary KoZHAF aircraft register PDF."""
    pdf_url = _discover_pdf_url(session)
    logger.info("Downloading Hungary KoZHAF aircraft register from %s", pdf_url)
    ...
    with pdfplumber.open(io.BytesIO(resp.content)) as pdf:
        for page in pdf.pages:
            ...
            for row in table:
                ...
                raw_reg = cells[_COL_REGISTRATION] if len(cells) > _COL_REGISTRATION else ""
                normalized = _normalize_registration(raw_reg)
                if not normalized.startswith("HA-"):
                    continue
                records.append({
                    "registration": normalized,
                    "model": cells[_COL_MODEL] if len(cells) > _COL_MODEL else "",
                    "serial": cells[_COL_SERIAL] if len(cells) > _COL_SERIAL else "",
                    "year": cells[_COL_YEAR] if len(cells) > _COL_YEAR else "",
                    "owner_name": cells[_COL_OWNER_NAME] if len(cells) > _COL_OWNER_NAME else "",
                    "owner_address": cells[_COL_OWNER_ADDRESS] if len(cells) > _COL_OWNER_ADDRESS else "",
                    "operator_name": cells[_COL_OPERATOR_NAME] if len(cells) > _COL_OPERATOR_NAME else "",
                })

    logger.info("Parsed %d HA- rows from PDF.", len(records))
    return records
...
def _build_record(row: dict, icao_hex: str, registration: str) -> dict:
    ...
    owner_address = _WHITESPACE_RE.sub(" ", row.get("owner_address", "").strip())
    if owner_address:
...
def write_to_redis(rows: list[dict], r: redis_lib.Redis, ttl: int) -> int:
    """Write Hungary KoZHAF data to aircraft:detail keys in Redis. Returns count written."""
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = {
        "ids": "SkyFollower_runner_hu_kozhaf_registry",
        "name": f"SkyFollower {country_flag('HU')} Hungary KoZHAF Registry Runner",
        "manufacturer": "P5Software, LLC",
    }
    stats = [
        ("records_imported", "Hungary KoZHAF Records Imported", "mdi:airplane", "total_increasing", None),
        ("last_run_at", "Hungary KoZHAF Last Run At", "mdi:clock", None, None),
        ("last_run_status", "Hungary KoZHAF Last Run Status", "mdi:check-circle", None, None),
    ]
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            "state_topic": f"{MQTT_ROOT}/statistic/{name}",
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_hu_kozhaf_registry_{name}",
            "object_id": f"SkyFollower_runner_hu_kozhaf_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_hu_kozhaf_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
...
def main() -> None:
    ...
    try:
        ...
        status = "success"
        logger.info(
            "Hungary KoZHAF runner completed successfully. Records imported: %d",
            records_imported,
        )

    except Exception as exc:
        logger.error("Hungary KoZHAF runner failed: %s", exc, exc_info=True)
