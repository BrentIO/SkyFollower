...
logger = logging.getLogger("hr-ccaa-registry")
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        ...
        name=f"SkyFollower Croatia {country_flag('HR')} CCAA Registry Runner",
        model=f"Croatia {country_flag('HR')} CCAA Registry Runner",
        configuration_url="https://brentio.github.io/SkyFollower/runners/hr-ccaa-registry.html",
    )
    stats = [
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_hr_ccaa_registry_{name}",
            "object_id": f"SkyFollower_runner_hr_ccaa_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_hr_ccaa_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
