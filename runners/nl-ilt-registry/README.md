...
See `specs/data-dictionary.yaml` (`nl-ilt-registry` entry) for full column semantics and
...
Published once, at the end of a run, to `SkyFollower/runner/nl-ilt-registry/statistic/{name}` (all retained):
...
Home Assistant autodiscovery configs are also published (retained) to `homeassistant/sensor/SkyFollower_runner_nl_ilt_registry_{name}/config` for each of the three stats above.
