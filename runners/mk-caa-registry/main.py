#!/usr/bin/env python3
"""
SkyFollower North Macedonia CAA Data Runner
...
Data source index: https://www.caa.gov.mk/en/safety/airworthiness-and-aircraft-registration/
"""
...
logger = logging.getLogger("mk-caa-registry")

_INDEX_URL = "https://www.caa.gov.mk/en/safety/airworthiness-and-aircraft-registration/"
REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/mk-caa-registry"
BATCH_SIZE = 100

# Column positions (no header row)
_COL_REGISTRATION = 2
_COL_MODEL = 3
_COL_MANUFACTURER = 4
_COL_SERIAL = 5
_COL_OWNER_NAME = 6
_COL_OWNER_ADDRESS = 7
...
def _discover_pdf_url(session: requests.Session) -> str:
    """Scrape the CAA index page and return the href of the aircraft register PDF."""
    logger.info("Fetching North Macedonia CAA register index from %s", _INDEX_URL)
    ...
    for a in soup.find_all("a", href=True):
        href = a["href"]
        if "AIRCRAFT-REGISTER" in href.upper() and href.lower().endswith(".pdf"):
            if not href.startswith("http"):
                from urllib.parse import urljoin
                href = urljoin(_INDEX_URL, href)
            return href

    raise RuntimeError("No AIRCRAFT-REGISTER PDF link found on CAA index page.")


def download_and_parse(session: requests.Session) -> list[dict]:
    """Discover, download, and parse the North Macedonia CAA aircraft register PDF."""
    pdf_url = _discover_pdf_url(session)
    logger.info("Downloading North Macedonia CAA aircraft register from %s", pdf_url)
    ...
    with pdfplumber.open(io.BytesIO(resp.content)) as pdf:
        for page in pdf.pages:
            ...
            for row in table:
                ...
                reg = cells[_COL_REGISTRATION] if len(cells) > _COL_REGISTRATION else ""
                if not reg.startswith("Z3-"):
                    continue
                records.append({
                    "registration": reg,
                    "model": cells[_COL_MODEL] if len(cells) > _COL_MODEL else "",
                    "manufacturer": cells[_COL_MANUFACTURER] if len(cells) > _COL_MANUFACTURER else "",
    ...
    logger.info("Parsed %d Z3- rows from PDF.", len(records))
    return records
...
def write_to_redis(rows: list[dict], r: redis_lib.Redis, ttl: int) -> int:
    """Write North Macedonia CAA data to aircraft:detail keys in Redis. Returns count written."""
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        identifier="SkyFollower_runner_mk_caa_registry",
        name=f"SkyFollower North Macedonia {country_flag('MK')} CAA Registry Runner",
        model=f"North Macedonia {country_flag('MK')} CAA Registry Runner",
        configuration_url="https://brentio.github.io/SkyFollower/runners/mk-caa-registry.html",
    )
    stats = [
        ("records_imported", "North Macedonia CAA Registry Records Imported", "mdi:airplane", "total_increasing", None),
        ("last_run_at", "North Macedonia CAA Last Run At", "mdi:clock", None, None),
        ("last_run_status", "North Macedonia CAA Last Run Status", "mdi:check-circle", None, None),
    ]
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            "state_topic": f"{MQTT_ROOT}/statistic/{name}",
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_mk_caa_registry_{name}",
            "object_id": f"SkyFollower_runner_mk_caa_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_mk_caa_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
...
def main() -> None:
    ...
    try:
        ...
        status = "success"
        logger.info(
            "North Macedonia CAA runner completed successfully. Records imported: %d",
            records_imported,
        )

    except Exception as exc:
        logger.error("North Macedonia CAA runner failed: %s", exc, exc_info=True)
