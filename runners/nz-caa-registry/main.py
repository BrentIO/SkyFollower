...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        identifier="SkyFollower_runner_nz_caa_registry",
        name=f"SkyFollower {country_flag('NZ')} New Zealand CAA Registry Runner",
        model=f"{country_flag('NZ')} New Zealand CAA Registry Runner",
    )
    stats = [
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_nz_caa_registry_{name}",
            "object_id": f"SkyFollower_runner_nz_caa_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_nz_caa_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
