...
logger = logging.getLogger("is-samgongustofa-registry")
...
REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/is-samgongustofa-registry"
BATCH_SIZE = 100
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_is_samgongustofa_registry_{name}",
            "object_id": f"SkyFollower_runner_is_samgongustofa_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_is_samgongustofa_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
