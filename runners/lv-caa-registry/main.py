#!/usr/bin/env python3
"""
SkyFollower Latvia CAA Data Runner
...
logger = logging.getLogger("lv-caa-registry")

_API_URL = (
    "https://data.gov.lv/dati/lv/api/action/datastore_search"
    "?resource_id=dbde00e6-8616-449a-8cac-ef748c6793f3&limit=50000"
)

REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/lv-caa-registry"
BATCH_SIZE = 100
...
def download_and_parse(session: requests.Session) -> list[dict]:
    """Fetch Latvia CAA aircraft register from the CKAN API and return records."""
    logger.info("Downloading Latvia CAA aircraft register from %s", _API_URL)
    ...
    for rec in raw_records:
        registration = (rec.get("Registration_Mark") or "").strip()
        if not registration.startswith("YL-"):
            continue
        records.append({
...
def _build_record(row: dict, icao_hex: str, registration: str) -> dict:
    """Build detail enrichment record from a parsed API row."""
    ...
    if year is not None:
        try:
            ...
            if 1900 <= year_int <= 2100:
                aircraft_fields["manufactured_date"] = f"{year_int}-01-01"
...
def write_to_redis(rows: list[dict], r: redis_lib.Redis, ttl: int) -> int:
    """Write Latvia CAA data to aircraft:detail keys in Redis. Returns count written."""
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        identifier="SkyFollower_runner_lv_caa_registry",
        name=f"SkyFollower Latvia {country_flag('LV')} CAA Registry Runner",
        model=f"Latvia {country_flag('LV')} CAA Registry Runner",
        configuration_url="https://brentio.github.io/SkyFollower/runners/lv-caa-registry.html",
    )
    stats = [
        ("records_imported", "Latvia CAA Registry Records Imported", "mdi:airplane", "total_increasing", None),
        ("last_run_at", "Latvia CAA Last Run At", "mdi:clock", None, None),
        ("last_run_status", "Latvia CAA Last Run Status", "mdi:check-circle", None, None),
    ]
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            "state_topic": f"{MQTT_ROOT}/statistic/{name}",
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_lv_caa_registry_{name}",
            "object_id": f"SkyFollower_runner_lv_caa_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_lv_caa_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
...
def main() -> None:
    ...
    try:
        ...
        status = "success"
        logger.info(
            "Latvia CAA runner completed successfully. Records imported: %d",
            records_imported,
        )

    except Exception as exc:
        logger.error("Latvia CAA runner failed: %s", exc, exc_info=True)
