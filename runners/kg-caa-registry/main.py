...
logger = logging.getLogger("kg-caa-registry")

_PAGE_URL = "https://caa.kg/en/node/46"

REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/kg-caa-registry"
BATCH_SIZE = 100
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        identifier="SkyFollower_runner_kg_caa_registry",
        name=f"SkyFollower Kyrgyzstan {country_flag('KG')} CAA Registry Runner",
        model=f"Kyrgyzstan {country_flag('KG')} CAA Registry Runner",
        configuration_url="https://brentio.github.io/SkyFollower/runners/kg-caa-registry.html",
    )
    stats = [
        ("records_imported", "Kyrgyzstan CAA Registry Records Imported", "mdi:airplane", "total_increasing", None),
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_kg_caa_registry_{name}",
            "object_id": f"SkyFollower_runner_kg_caa_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_kg_caa_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
