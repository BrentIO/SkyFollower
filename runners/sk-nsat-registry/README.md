...
See `specs/data-dictionary.yaml` (`sk-nsat-registry` entry) for full column semantics and cross-source schema notes.
...
Published once, at the end of a run, to `SkyFollower/runner/sk-nsat-registry/statistic/{name}` (all retained):
