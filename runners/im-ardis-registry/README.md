...
See `specs/data-dictionary.yaml` (`im-ardis-registry` entry) for full column semantics and cross-source schema notes.
...
Published once, at the end of a run, to `SkyFollower/runner/im-ardis-registry/statistic/{name}` (all retained):
...
Home Assistant autodiscovery configs are also published (retained) to `homeassistant/sensor/SkyFollower_runner_im_ardis_registry_{name}/config` for each of the three stats above.
