...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "im_ardis_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["im_ardis_registry_main"] = mod
...
class TestBuildRecord:
    def test_full_record(self):
        ...
        assert record["source"] == "im-ardis-registry"
...
class TestWriteToRedis:
    ...
    def test_source_field_in_written_record(self):
        ...
        write_to_redis(rows, r, REDIS_TTL)
        set_call = r.pipeline.return_value.json.return_value.set.call_args
        assert set_call[0][2]["source"] == "im-ardis-registry"
...
class TestTypeDesignatorConsensusDeduction:
    def _rows(self):
        ...
        return [
            _make_row(registration="M-0001", mode_s="AAAA01", manufacturer="Piper", model="PA-28-181"),
            _make_row(registration="M-0002", mode_s="AAAA02", manufacturer="Piper", model="PA-28-181"),
            _make_row(registration="M-0003", mode_s="AAAA03", manufacturer="Piper", model="PA-28-181"),
            _make_row(registration="M-0004", mode_s="AAAA04", manufacturer="Piper", model="PA-28-181"),
        ]
...
class TestPublishCompletionStats:
    ...
    def test_mqtt_connect_timeout_does_not_raise(self):
        ...
        with patch("im_ardis_registry_main.mqtt.Client") as mock_cls:
            mock_client = MagicMock()
    ...
    def test_mqtt_publishes_records_imported(self):
        ...
        with patch("im_ardis_registry_main.mqtt.Client", return_value=mc):
    ...
    def test_mqtt_root_topic(self):
        assert MQTT_ROOT == "SkyFollower/runner/im-ardis-registry"
