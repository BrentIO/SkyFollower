#!/usr/bin/env python3
"""
SkyFollower Maldives CAA Data Runner

Fetches the Maldives Civil Aviation Authority aircraft register by scraping the
...
PDF columns (0-based):
  ...
  3:  Manufacturer+Designation (stored as aircraft.model; newlines collapsed)
  4:  MTOW (not stored)
  5:  Serial Number (stored as aircraft.serial_number)
  6:  Year Built (stored as aircraft.manufactured_date → YYYY-01-01)
  7:  Owner Name+Address (first line → registrant.names[0]; remaining → registrant.street)
...
Data source index: https://www.caa.gov.mv/operations/registration-of-aircraft-and-mortgages
"""
...
logger = logging.getLogger("mv-caa-registry")

_INDEX_URL = "https://www.caa.gov.mv/operations/registration-of-aircraft-and-mortgages"

REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/mv-caa-registry"
BATCH_SIZE = 100
...
def _discover_pdf_url(session: requests.Session) -> str:
    """Scrape the CAA index page and return the PDF href."""
    logger.info("Fetching Maldives CAA register index from %s", _INDEX_URL)
    ...
    for a in soup.find_all("a", href=True):
        href = a["href"]
        if "caa.gov.mv" in href and href.lower().endswith(".pdf"):
            return href
        if href.lower().endswith(".pdf") and not href.startswith("http"):
            from urllib.parse import urljoin
            return urljoin(_INDEX_URL, href)

    raise RuntimeError("No PDF link found on Maldives CAA index page.")


def download_and_parse(session: requests.Session) -> list[dict]:
    """Discover, download, and parse the Maldives CAA aircraft register PDF."""
    pdf_url = _discover_pdf_url(session)
    logger.info("Downloading Maldives CAA aircraft register from %s", pdf_url)
        ...
        for page in pdf.pages:
            ...
            for row in table:
                ...
                if len(cells) <= _COL_STATUS:
                    continue
                reg = cells[_COL_REGISTRATION]
                if not reg.startswith("8Q-"):
                    continue
                ...
                records.append({
                    ...
                    "year_built": cells[_COL_YEAR],
                    "owner": cells[_COL_OWNER],
                })

    logger.info("Parsed %d valid 8Q- rows from PDF.", len(records))
    return records
...
def write_to_redis(rows: list[dict], r: redis_lib.Redis, ttl: int) -> int:
    """Write Maldives CAA data to aircraft:detail keys in Redis. Returns count written."""
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        identifier="SkyFollower_runner_mv_caa_registry",
        name=f"SkyFollower Maldives {country_flag('MV')} CAA Registry Runner",
        model=f"Maldives {country_flag('MV')} CAA Registry Runner",
        configuration_url="https://brentio.github.io/SkyFollower/runners/mv-caa-registry.html",
    )
    stats = [
        ("records_imported", "Maldives CAA Registry Records Imported", "mdi:airplane", "total_increasing", None),
        ("last_run_at", "Maldives CAA Last Run At", "mdi:clock", None, None),
        ("last_run_status", "Maldives CAA Last Run Status", "mdi:check-circle", None, None),
    ]
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            "state_topic": f"{MQTT_ROOT}/statistic/{name}",
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_mv_caa_registry_{name}",
            "object_id": f"SkyFollower_runner_mv_caa_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_mv_caa_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
...
def main() -> None:
    ...
    try:
        ...
        status = "success"
        logger.info(
            "Maldives CAA runner completed successfully. Records imported: %d",
            records_imported,
        )

    except Exception as exc:
        logger.error("Maldives CAA runner failed: %s", exc, exc_info=True)
