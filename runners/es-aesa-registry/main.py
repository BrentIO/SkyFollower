...
logger = logging.getLogger("es-aesa-registry")

PDF_URL = "https://www.seguridadaerea.gob.es/sites/default/files/aeronaves_inscritas.pdf"
REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/es-aesa-registry"
BATCH_SIZE = 100
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        ...
        name=f"SkyFollower {country_flag('ES')} Spain AESA Registry Runner",
        model=f"{country_flag('ES')} Spain AESA Registry Runner",
    )
    stats = [
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_es_aesa_registry_{name}",
            "object_id": f"SkyFollower_runner_es_aesa_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_es_aesa_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
