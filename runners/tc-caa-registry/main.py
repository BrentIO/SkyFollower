#!/usr/bin/env python3
"""
SkyFollower Turks & Caicos CAA Data Runner

Fetches the Turks & Caicos Islands Civil Aviation Authority (CAA) aircraft
...
from Mictronics. Schedule it AFTER the Mictronics runner.
...
logger = logging.getLogger("tc-caa-registry")

REGISTER_URL = "https://tcicaa.tc/operations-safety/aircrafts/aircraft-register"
REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/tc-caa-registry"
BATCH_SIZE = 100
...
def download_and_parse(session: requests.Session) -> list[dict]:
    """Fetch the Turks & Caicos CAA register page and parse the HTML table."""
    logger.info("Downloading Turks & Caicos CAA aircraft register from %s", REGISTER_URL)
    ...
    for candidate in soup.find_all("table"):
        ...
        if first_row and "REG" in first_row.get_text():
            table = candidate
            break

    if table is None:
        raise RuntimeError("Could not find aircraft register table on Turks & Caicos CAA page.")

    rows = table.find_all("tr")
    if not rows:
        raise RuntimeError("Register table on Turks & Caicos CAA page is empty.")
...
def _build_record(row: dict, icao_hex: str, registration: str) -> dict:
    ...
    manufacturer = row.get("MANUFACT.", "").strip()
    if manufacturer:
    ...
    serial = row.get("S/N", "").strip()
    if serial:
        aircraft_fields["serial_number"] = serial

    owner = row.get("OWNER /OPERATOR", "").strip()
    if owner:
...
def write_to_redis(rows: list[dict], r: redis_lib.Redis, ttl: int) -> int:
    """Write Turks & Caicos CAA data to aircraft:detail keys in Redis. Returns count written."""
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        identifier="SkyFollower_runner_tc_caa_registry",
        name=f"SkyFollower Turks & Caicos {country_flag('TC')} CAA Registry Runner",
        model=f"Turks & Caicos {country_flag('TC')} CAA Registry Runner",
        configuration_url="https://brentio.github.io/SkyFollower/runners/tc-caa-registry.html",
    )
    stats = [
        ("records_imported", "Turks & Caicos CAA Registry Records Imported", "mdi:airplane", "total_increasing", None),
        ("last_run_at", "Turks & Caicos CAA Last Run At", "mdi:clock", None, None),
        ("last_run_status", "Turks & Caicos CAA Last Run Status", "mdi:check-circle", None, None),
    ]
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            "state_topic": f"{MQTT_ROOT}/statistic/{name}",
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_tc_caa_registry_{name}",
            "object_id": f"SkyFollower_runner_tc_caa_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_tc_caa_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
...
def main() -> None:
    ...
    try:
        ...
        status = "success"
        logger.info(
            "Turks & Caicos CAA runner completed successfully. Records imported: %d",
            records_imported,
        )

    except Exception as exc:
        logger.error("Turks & Caicos CAA runner failed: %s", exc, exc_info=True)
