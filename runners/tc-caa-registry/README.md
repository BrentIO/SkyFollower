...
See `specs/data-dictionary.yaml` (`tc-caa-registry` entry) for full column semantics and cross-source schema notes.
...
Published once, at the end of a run, to `SkyFollower/runner/tc-caa-registry/statistic/{name}` (all retained):
...
Home Assistant autodiscovery configs are also published (retained) to `homeassistant/sensor/SkyFollower_runner_tc_caa_registry_{name}/config` for each of the three stats above.
