...
logger = logging.getLogger("br-anac-registry")
...
def write_to_redis(records: list[dict], r: redis_lib.Redis, ttl: int) -> int:
    ...
    for registration, icao_hex in icao_map.items():
        ...
        try:
            ...
            record["source"] = "br-anac-registry"
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        ...
        name=f"SkyFollower {country_flag('BR')} Brazil ANAC Registry Runner",
        model=f"{country_flag('BR')} Brazil ANAC Registry Runner",
    )
    stats = [
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_br_anac_registry_{name}",
            "object_id": f"SkyFollower_runner_br_anac_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
