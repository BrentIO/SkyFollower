...
| `AircraftDetails.ICAOAircraftTypeDesignator` | ✅ | → `aircraft.type_designator`; also used to look up `aircraft:type:{type_designator}` in Redis, setting `aircraft.manufacturer_model` and `aircraft.wake_turbulence_category` when found |
...
See `specs/data-dictionary.yaml` (`uk-caa-registry` entry) for full column semantics and cross-source schema notes.
...
Published once, at the end of a run, to `SkyFollower/runner/uk-caa-registry/statistic/{name}` (all retained):
...
Home Assistant autodiscovery configs are also published (retained) to `homeassistant/sensor/SkyFollower_runner_uk_caa_registry_{name}/config` for each of the three stats above.
