...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    device = build_ha_device(
        ...
        name=f"SkyFollower {country_flag('LU')} Luxembourg DAC Registry Runner",
        model=f"{country_flag('LU')} Luxembourg DAC Registry Runner",
    )
    stats = [
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_lu_dac_registry_{name}",
            "object_id": f"SkyFollower_runner_lu_dac_registry_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_lu_dac_registry_{name}/config",
            json.dumps(payload),
            retain=True,
        )
