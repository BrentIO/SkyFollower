...
and external feed, not one antenna. Requires a configured center
(`MAP_CENTER_LATITUDE`/`LONGITUDE`) -- there's no origin to measure from
...
Snapshots written while the bearing bucket resolution was briefly doubled
to 720 half-degree buckets (#1683, reverted by #1805) store bucket keys
under that half-degree scheme. A syntactically valid old key like `"47"`
is silently reinterpreted under the reverted (360-bucket) code as 47°, not
half-degree index 47 (23.5°). This is a known, accepted limitation rather
than something migrated: a historical `?date=` lookup against a snapshot
from that window may render with shifted bearings for the remainder of
that snapshot's 30-day retention window, after which it's deleted and the
