...
3. A FastAPI app exposing `GET /api/flights` (a snapshot), `GET
   ...
   batched relay of `position`/`metadata`/`stale`/`remove` events), and
   serving the built frontend SPA itself under `/map` (see
...
npm test          # vitest -- altitudeColor, info-box formatting, label stack-order, and processor-status unit tests
```
