...
| `--sources` | Yes | One or more `HOST:PORT:SOURCE_TAG` triples, space-separated. `SOURCE_TAG` must be exactly one of `1090`, `978`, or `MLAT` (case-sensitive). Use `1090` for a readsb raw 1090 MHz port, `978` for a dump978-fa raw UAT port, and `MLAT` for a readsb MLAT-derived port (parsed the same way as `1090` — MLAT positions arrive as ordinary Mode S frames, just tagged by source rather than by frame content). |
...
| `source` | string | `"1090"`, `"978"`, or `"MLAT"` — whichever source tag was configured for the connection this message arrived on. |

1090/MLAT frames are decoded once via pyModeS purely to extract `icao_hex`
...
that an `MLAT`-tagged source is routed through the same decode path as
`1090`.
