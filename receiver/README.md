...
With `REDIS_HOST` configured, each `sources[]` connection also gets two cross-restart-durable cumulative message counts -- `messages_{host}_{port}_total_hour` and `_total_today` -- for a source too sparse for the existing 30-second `messages_{host}_{port}_per_second` rate to say anything useful about. A third count, `messages_{host}_{port}_total_lifetime`, is always published (Redis or not) but is a different kind of figure -- see below.

The per-message hot path (`_RateTracker.record()`) stays pure in-memory arithmetic; nothing there ever touches Redis or checks the clock. The hour/today counts are flushed to Redis from the same background thread that already handles telemetry, on the fixed `MQTT_PUBLISH_INTERVAL_SECONDS` cadence -- purely time-based, with no message-count trigger, so a burst of traffic just produces one larger `INCRBY` on the next tick rather than an early flush. The actual increment-with-expiry-only-on-creation is `shared/lua/incr_period_counter.lua`, shared with the message processor's own equivalent counters -- called via `EVALSHA` so the exists-check, increment, and conditional `EXPIREAT` are one atomic round-trip.

**The receiver does not publish the hour/today topics itself.** It only feeds the Redis counters; **`core-health`** reads those and is the sole publisher of both the `messages_*_total_{hour,today}` MQTT values and their Home Assistant discovery config. That keeps one owner per topic -- previously the receiver *also* published them from its own in-memory running totals, and two retained publishers on the same topic, on two unsynchronized 30-second timers, made the sensor flap between a Redis-accurate and an in-memory value. As a consequence, **with `REDIS_HOST` unset there is no `core-health` path and those two sensors simply don't exist** -- the per-second rate, the per-connection state topics, and the lifetime counter below are unaffected either way.

**`messages_{host}_{port}_total_lifetime` is by design a device-local, resets-on-restart figure.** It is a running total for *this receiver process's own lifetime*, held only in memory, never written to or read from Redis, and published directly by the receiver the same way `messages_{host}_{port}_per_second` is. Restarting a receiver container resets it to zero for every connection on that receiver -- that is intended: it answers "how many messages has this connection handled since it last came up", not "since the receiver was first installed". Its HA discovery uses `state_class: total_increasing`, the correct semantics for a counter that legitimately resets on a device restart. The hour/today counters are unaffected by a restart (still Redis-backed, still correct across it).
...
| `messages_{host}_{port}_total_lifetime` | Integer as string | Cumulative message count for that connection since **this receiver process** started — an in-memory figure, never Redis-backed, that resets to `0` on every receiver restart by design. See [Redis-Backed Message Counters](#redis-backed-message-counters) |
...
`messages_{host}_{port}_total_{hour,today}` is **not** in this table -- `core-health` publishes those two (value and HA discovery) from the Redis counters the receiver's flush feeds; see [Redis-Backed Message Counters](#redis-backed-message-counters). The sibling `_total_lifetime` topic above *is* the receiver's own.
