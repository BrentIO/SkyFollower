...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "me_caa_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["me_caa_registry_main"] = mod
...
class TestFetchListPage:
    ...
    def test_logs_url(self):
        ...
        session.get.return_value = _make_response(_list_html([]))
        import logging
        with patch.object(logging.getLogger("me-caa-registry"), "info") as mock_log:
...
class TestFetchDetailPage:
    ...
    def test_retries_on_403(self):
        ...
        with patch("me_caa_registry_main.time.sleep"):
            detail = _fetch_detail_page(session, "4O-AOA")
    ...
    def test_logs_url(self):
        ...
        session.get.return_value = _make_response(_detail_html())
        import logging
        with patch.object(logging.getLogger("me-caa-registry"), "debug") as mock_log:
            _fetch_detail_page(session, "4O-AOA")
        logged = " ".join(str(a) for call in mock_log.call_args_list for a in call.args)
...
class TestBuildRecord:
    def test_full_record(self):
        ...
        assert record["source"] == "me-caa-registry"
...
class TestWriteToRedis:
    ...
    def test_source_field_in_written_record(self):
        ...
        write_to_redis(rows, r, REDIS_TTL)
        set_call = r.pipeline.return_value.json.return_value.set.call_args
        assert set_call[0][2]["source"] == "me-caa-registry"
...
class TestPublishCompletionStats:
    ...
    def test_mqtt_connect_timeout_does_not_raise(self):
        ...
        with patch("me_caa_registry_main.mqtt.Client") as mock_cls:
            mock_client = MagicMock()
    ...
    def test_mqtt_publishes_records_imported(self):
        ...
        with patch("me_caa_registry_main.mqtt.Client", return_value=mc):
    ...
    def test_mqtt_root_topic(self):
        assert MQTT_ROOT == "SkyFollower/runner/me-caa-registry"
...
class TestNetwork:
    ...
    def test_url_reachable(self):
        assert_url_reachable(_mod._LIST_URL, "me-caa-registry", headers={"User-Agent": _mod._BROWSER_UA, "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"})
