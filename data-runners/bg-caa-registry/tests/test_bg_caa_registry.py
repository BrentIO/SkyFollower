...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "bg_caa_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["bg_caa_registry_main"] = mod
...
class TestBuildRecord:
    def test_basic_fields(self):
        ...
        assert record["source"] == "bg-caa-registry"
...
class TestNetwork:
    ...
    def test_url_reachable(self):
        assert_url_reachable(_mod._INDEX_URL, "bg-caa-registry", headers={"User-Agent": "Mozilla/5.0 (compatible; P5Software SkyFollower)"})
