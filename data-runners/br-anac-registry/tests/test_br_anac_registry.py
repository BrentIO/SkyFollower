...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "br_anac_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["br_anac_registry_main"] = mod
...
class TestWriteToRedis:
    ...
    def _patch_reg_map(self, reg_map: dict):
        return patch("br_anac_registry_main._build_registration_map", return_value=reg_map)
    ...
    def test_writes_source_field(self):
        ...
        with self._patch_reg_map({"PP-AJH": "E491A0"}):
            write_to_redis(rows, r, REDIS_TTL)
        written = r.json.return_value.set.call_args.args[2]
        assert written["source"] == "br-anac-registry"
...
class TestMqttCompletionStats:
    ...
    def test_publishes_records_imported(self):
        ...
        with patch("br_anac_registry_main.mqtt.Client", return_value=mc):
    ...
    def test_ha_autodiscovery_three_sensors(self):
        ...
        assert "homeassistant/sensor/SkyFollower_runner_br_anac_registry_records_imported/config" in ha_topics
...
class TestNetwork:
    ...
    def test_url_reachable(self):
        assert_url_reachable(_mod.DOWNLOAD_URL, "br-anac-registry", headers={"User-Agent": "P5Software SkyFollower"})
