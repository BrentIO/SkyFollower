...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "cz_caa_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["cz_caa_registry_main"] = mod
...
class TestFetchActiveIds:
    ...
    def test_logs_list_url(self):
        ...
        session.get.return_value = _list_response([])
        import logging
        with patch.object(logging.getLogger("cz-caa-registry"), "info") as mock_log:
            _fetch_active_ids(session)
...
class TestFetchDetail:
    ...
    def test_retries_on_connection_error_then_succeeds(self):
        ...
        with patch("cz_caa_registry_main.time.sleep"):
            result = _fetch_detail(session, 1)
    ...
    def test_retry_uses_exponential_backoff(self):
        ...
        with patch("cz_caa_registry_main.time.sleep") as mock_sleep:
    ...
    def test_logs_detail_url(self):
        ...
        session.get.return_value = _detail_response()
        import logging
        with patch.object(logging.getLogger("cz-caa-registry"), "debug") as mock_log:
...
class TestBuildRecord:
    def test_full_record(self):
        ...
        assert record["source"] == "cz-caa-registry"
...
class TestWriteToRedis:
    ...
    def test_source_field_in_record(self):
        ...
        assert written["source"] == "cz-caa-registry"
...
class TestPublishCompletionStats:
    ...
    def test_mqtt_connect_timeout_does_not_raise(self):
        ...
        with patch("cz_caa_registry_main.mqtt.Client") as mock_cls:
    ...
    def test_mqtt_publishes_records_imported(self):
        ...
        with patch("cz_caa_registry_main.mqtt.Client", return_value=mc):
    ...
    def test_mqtt_root_topic(self):
        assert MQTT_ROOT == "SkyFollower/runner/cz-caa-registry"
...
class TestNetwork:
    ...
    def test_url_reachable(self):
        assert_url_reachable(_mod._LIST_URL, "cz-caa-registry", headers={"User-Agent": "Mozilla/5.0 (compatible; P5Software SkyFollower)"})
