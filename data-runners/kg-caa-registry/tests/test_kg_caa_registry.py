...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "kg_caa_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["kg_caa_registry_main"] = mod
...
class TestDownloadAndParse:
    ...
    def test_logs_url(self):
        ...
        session.get.return_value = _make_response("", 503)
        import logging
        with patch.object(logging.getLogger("kg-caa-registry"), "info") as mock_log:
...
class TestBuildRecord:
    def test_full_record(self):
        ...
        assert record["source"] == "kg-caa-registry"
...
class TestWriteToRedis:
    ...
    def test_source_field_in_written_record(self):
        ...
        write_to_redis(rows, r, REDIS_TTL)
        set_call = r.pipeline.return_value.json.return_value.set.call_args
        assert set_call[0][2]["source"] == "kg-caa-registry"
...
class TestPublishCompletionStats:
    ...
    def test_mqtt_connect_timeout_does_not_raise(self):
        ...
        with patch("kg_caa_registry_main.mqtt.Client") as mock_cls:
            mock_client = MagicMock()
    ...
    def test_mqtt_publishes_records_imported(self):
        ...
        with patch("kg_caa_registry_main.mqtt.Client", return_value=mc):
    ...
    def test_mqtt_root_topic(self):
        assert MQTT_ROOT == "SkyFollower/runner/kg-caa-registry"
...
class TestNetwork:
    ...
    def test_url_reachable(self):
        assert_url_reachable(_mod._PAGE_URL, "kg-caa-registry", headers={"User-Agent": "P5Software SkyFollower"})
