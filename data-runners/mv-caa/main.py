...
REDIS_TTL = 14 * 86400
MQTT_ROOT = "SkyFollower/runner/mv-caa"
BATCH_SIZE = 100
...
def _publish_ha_autodiscovery(client: mqtt.Client) -> None:
    ...
    for name, friendly_name, icon, state_class, unit in stats:
        payload: dict = {
            ...
            "name": friendly_name,
            "unique_id": f"SkyFollower_runner_mv_caa_{name}",
            "object_id": f"SkyFollower_runner_mv_caa_{name}",
            "device": device,
            "icon": icon,
        }
        if state_class:
        ...
        client.publish(
            f"homeassistant/sensor/SkyFollower_runner_mv_caa_{name}/config",
            json.dumps(payload),
            retain=True,
        )
