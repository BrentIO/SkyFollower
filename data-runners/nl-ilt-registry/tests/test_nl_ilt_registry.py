...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "nl_ilt_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["nl_ilt_registry_main"] = mod
...
class TestFindDownloadUrl:
    def test_finds_absolute_url(self):
        ...
        with patch("nl_ilt_registry_main.requests.get") as mock_get:
            resp = MagicMock()
...
class TestApplyTypeLookup:
    def _make_redis_with_type(self, type_doc=None) -> MagicMock:
        r = MagicMock()
    ...
    def test_lookup_key_uses_type_designator(self):
        ...
        _apply_type_lookup(record, r)
        r.json.return_value.get.assert_called_once_with("aircraft:type:C152")

    def test_write_happens_even_if_mictronics_manufacturer_model_already_present(self):
        record = {"aircraft": {"type_designator": "C152", "manufacturer_model": "STALE VALUE"}}
        r = self._make_redis_with_type({"manufacturer_model": "CESSNA 152"})
        _apply_type_lookup(record, r)
        assert record["aircraft"]["manufacturer_model"] == "CESSNA 152"

    def test_designator_not_found_leaves_fields_unset(self):
        record = {"aircraft": {"type_designator": "BALL"}}
    ...
    def test_no_type_designator_skips_lookup_entirely(self):
        ...
        r = self._make_redis_with_type({"manufacturer_model": "SHOULD NOT BE USED"})
        _apply_type_lookup(record, r)
        assert "manufacturer_model" not in record["aircraft"]
        r.json.return_value.get.assert_not_called()

    def test_no_aircraft_object_does_not_crash(self):
        record = {"icao_hex": "4B1234"}
        r = self._make_redis_with_type({"manufacturer_model": "SHOULD NOT BE USED"})
        _apply_type_lookup(record, r)
...
class TestWriteToRedis:
    ...
    def test_manufacturer_model_set_from_type_lookup_end_to_end(self):
        rows = [_make_row(icao_hex="4B1234", icao_code="C172")]
        r = _make_redis()
    ...
    def test_source_field_in_written_record(self):
        ...
        write_to_redis(rows, r, REDIS_TTL)
        set_call = r.pipeline.return_value.json.return_value.set.call_args
        assert set_call[0][2]["source"] == "nl-ilt-registry"
...
class TestDownloadRegistry:
    def test_writes_temp_file(self):
        with patch("nl_ilt_registry_main._find_download_url", return_value="https://www.ilent.nl/x.ods"):
            with patch("nl_ilt_registry_main.requests.get") as mock_get:
                resp = MagicMock()
                resp.status_code = 200
...
class TestPublishCompletionStats:
    ...
    def test_mqtt_connect_timeout_does_not_raise(self):
        ...
        with patch("nl_ilt_registry_main.mqtt.Client") as mock_cls:
            mock_client = MagicMock()
    ...
    def test_mqtt_publishes_records_imported(self):
        ...
        with patch("nl_ilt_registry_main.mqtt.Client", return_value=mc):
    ...
    def test_mqtt_root_topic(self):
        assert MQTT_ROOT == "SkyFollower/runner/nl-ilt-registry"
...
class TestNetwork:
    ...
    def test_url_reachable(self):
        assert_url_reachable(_mod.INDEX_URL, "nl-ilt-registry", headers={"User-Agent": "P5Software SkyFollower"})
