...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "sg_caas_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["sg_caas_registry_main"] = mod
...
class TestBuildRecord:
    def test_full_record(self):
        ...
        assert record["source"] == "sg-caas-registry"
...
class TestWriteToRedis:
    ...
    def test_source_field_in_written_record(self):
        ...
        write_to_redis(rows, r, REDIS_TTL)
        set_call = r.pipeline.return_value.json.return_value.set.call_args
        assert set_call[0][2]["source"] == "sg-caas-registry"
...
class TestDownloadAndParse:
    ...
    def test_parses_9v_rows(self):
        ...
        from sg_caas_registry_main import download_and_parse
    ...
    def test_referer_sent_on_xlsx_download(self):
        ...
        from sg_caas_registry_main import download_and_parse, _INDEX_URL
        download_and_parse(session)
...
class TestPublishCompletionStats:
    ...
    def test_mqtt_connect_timeout_does_not_raise(self):
        ...
        with patch("sg_caas_registry_main.mqtt.Client") as mock_cls:
            mock_client = MagicMock()
    ...
    def test_mqtt_publishes_records_imported(self):
        ...
        with patch("sg_caas_registry_main.mqtt.Client", return_value=mc):
    ...
    def test_mqtt_root_topic(self):
        assert MQTT_ROOT == "SkyFollower/runner/sg-caas-registry"
...
class TestNetwork:
    ...
    def test_url_reachable(self):
        assert_url_reachable(_mod._INDEX_URL, "sg-caas-registry", headers={"User-Agent": _mod._BROWSER_UA, "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"})
