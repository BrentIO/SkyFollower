...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "md_caa_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["md_caa_registry_main"] = mod
...
class TestBuildRecord:
    def test_full_record(self):
        ...
        assert record["source"] == "md-caa-registry"
...
class TestDownloadAndParse:
    ...
    def test_logs_pdf_url(self):
        ...
        session.get.return_value = resp
        import logging
        with patch.object(logging.getLogger("md-caa-registry"), "info") as mock_log:
    ...
    def test_filters_non_er_rows(self):
        ...
        with patch("md_caa_registry_main.pdfplumber.open", return_value=_make_pdf([page])):
    ...
    def test_skips_page_with_no_table(self):
        ...
        with patch("md_caa_registry_main.pdfplumber.open", return_value=_make_pdf([page, page2])):
            records = download_and_parse(session)
        assert len(records) == 1
    ...
    def test_multiple_pages_combined(self):
        ...
        with patch("md_caa_registry_main.pdfplumber.open", return_value=_make_pdf([page1, page2])):
            records = download_and_parse(session)
...
class TestWriteToRedis:
    ...
    def test_source_field_in_written_record(self):
        ...
        write_to_redis(rows, r, REDIS_TTL)
        set_call = r.pipeline.return_value.json.return_value.set.call_args
        assert set_call[0][2]["source"] == "md-caa-registry"
...
class TestPublishCompletionStats:
    ...
    def test_mqtt_connect_timeout_does_not_raise(self):
        ...
        with patch("md_caa_registry_main.mqtt.Client") as mock_cls:
            mock_client = MagicMock()
    ...
    def test_mqtt_publishes_records_imported(self):
        ...
        with patch("md_caa_registry_main.mqtt.Client", return_value=mc):
    ...
    def test_mqtt_root_topic(self):
        assert MQTT_ROOT == "SkyFollower/runner/md-caa-registry"
...
class TestNetwork:
    ...
    def test_url_reachable(self):
        assert_url_reachable(_mod._PDF_URL, "md-caa-registry", headers={"User-Agent": "P5Software SkyFollower"})
