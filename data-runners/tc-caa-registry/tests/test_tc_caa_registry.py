"""Tests for the Turks & Caicos CAA data runner."""
...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "tc_caa_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["tc_caa_registry_main"] = mod
...
class TestBuildRecord:
    def test_full_record(self):
        row = _make_row()
        record = _build_record(row, "0C1234", "VQ-TCA")
        assert record["icao_hex"] == "0C1234"
        assert record["registration"] == "VQ-TCA"
        assert record["source"] == "tc-caa-registry"
    ...
    def test_dor_not_stored(self):
        row = _make_row(dor="2020-01-01")
        record = _build_record(row, "0C1234", "VQ-TCA")
        assert "D.O.R" not in record
...
class TestEscapeTag:
    def test_plain_value_unchanged(self):
        assert _escape_tag("VQTCA") == "VQTCA"

    def test_hyphen_escaped(self):
        assert "\\-" in _escape_tag("VQ-TCA")
...
class TestWriteToRedis:
    ...
    def test_source_field_in_written_record(self):
        ...
        write_to_redis(rows, r, REDIS_TTL)
        set_call = r.pipeline.return_value.json.return_value.set.call_args
        assert set_call[0][2]["source"] == "tc-caa-registry"
...
class TestPublishCompletionStats:
    ...
    def test_mqtt_connect_timeout_does_not_raise(self):
        ...
        with patch("tc_caa_registry_main.mqtt.Client") as mock_cls:
            mock_client = MagicMock()
    ...
    def test_mqtt_publishes_records_imported(self):
        ...
        with patch("tc_caa_registry_main.mqtt.Client", return_value=mc):
    ...
    def test_mqtt_records_imported_value(self):
        ...
        with patch("tc_caa_registry_main.mqtt.Client", return_value=mc):
            with patch("time.sleep"):
                publish_completion_stats(cfg, 28, "success")
        calls = {c.args[0]: c.args[1] for c in mc.publish.call_args_list}
        assert calls[f"{MQTT_ROOT}/statistic/records_imported"] == "28"
    ...
    def test_mqtt_root_topic(self):
        assert MQTT_ROOT == "SkyFollower/runner/tc-caa-registry"
...
class TestNetwork:
    ...
    def test_url_reachable(self):
        assert_url_reachable(_mod.REGISTER_URL, "tc-caa-registry", headers={"User-Agent": "P5Software SkyFollower"})
