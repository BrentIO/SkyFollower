...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "is_samgongustofa_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["is_samgongustofa_registry_main"] = mod
...
class TestBuildRecord:
    def test_full_record(self):
        ...
        assert record["source"] == "is-samgongustofa-registry"
...
class TestWriteToRedis:
    ...
    def test_source_field_in_written_record(self):
        ...
        assert written.get("source") == "is-samgongustofa-registry"
...
class TestPublishCompletionStats:
    ...
    def test_mqtt_connect_timeout_does_not_raise(self):
        ...
        with patch("is_samgongustofa_registry_main.mqtt.Client") as mock_cls:
            mock_client = MagicMock()
    ...
    def test_mqtt_publishes_records_imported(self):
        ...
        with patch("is_samgongustofa_registry_main.mqtt.Client", return_value=mc):
...
class TestNetwork:
    ...
    def test_url_reachable(self):
        assert_url_reachable(_mod.API_URL, "is-samgongustofa-registry", headers={"User-Agent": "P5Software SkyFollower"})
