...
_RUNNER_DIR = os.path.dirname(_HERE)        # data-runners/ca-transport-canada-registry/
...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "ca_transport_canada_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["ca_transport_canada_registry_main"] = mod
...
class TestWriteToRedis:
    ...
    def test_record_written_as_dict(self):
        ...
        assert record["source"] == "ca-transport-canada-registry"
        assert record["registration"] == "C-GABC"
        conn.close()

    def test_source_set_on_all_records(self):
        """Every record written to Redis carries source='ca-transport-canada-registry'."""
        conn = self._make_db()
        r, _, pipe_json = self._mock_redis()
        write_to_redis(conn, r, REDIS_TTL)
...
class TestMqttCompletionStats:
    ...
    def test_publishes_records_imported(self):
        ...
        with patch("ca_transport_canada_registry_main.mqtt.Client", return_value=mc):
