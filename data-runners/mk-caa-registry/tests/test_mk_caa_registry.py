...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "mk_caa_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["mk_caa_registry_main"] = mod
...
class TestBuildRecord:
    def test_full_record(self):
        ...
        assert record["source"] == "mk-caa-registry"
...
class TestDownloadAndParse:
    def test_raises_on_index_error(self):
        ...
        from mk_caa_registry_main import download_and_parse
    ...
    def test_logs_index_url(self):
        ...
        session.get.return_value = resp
        import logging
        from mk_caa_registry_main import download_and_parse, _INDEX_URL
        with patch.object(logging.getLogger("mk-caa-registry"), "info") as mock_log:
            with pytest.raises(RuntimeError):
                download_and_parse(session)
...
class TestWriteToRedis:
    ...
    def test_source_field_in_written_record(self):
        ...
        write_to_redis(rows, r, REDIS_TTL)
        set_call = r.pipeline.return_value.json.return_value.set.call_args
        assert set_call[0][2]["source"] == "mk-caa-registry"
...
class TestPublishCompletionStats:
    ...
    def test_mqtt_connect_timeout_does_not_raise(self):
        ...
        with patch("mk_caa_registry_main.mqtt.Client") as mock_cls:
            mock_client = MagicMock()
    ...
    def test_mqtt_publishes_records_imported(self):
        ...
        with patch("mk_caa_registry_main.mqtt.Client", return_value=mc):
    ...
    def test_mqtt_root_topic(self):
        assert MQTT_ROOT == "SkyFollower/runner/mk-caa-registry"
...
class TestNetwork:
    ...
    def test_url_reachable(self):
        assert_url_reachable(_mod._INDEX_URL, "mk-caa-registry", headers={"User-Agent": "P5Software SkyFollower"})
