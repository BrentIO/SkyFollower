...
def _load_main():
    spec = importlib.util.spec_from_file_location(
        "at_austrocontrol_registry_main",
        os.path.join(_RUNNER_DIR, "main.py"),
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["at_austrocontrol_registry_main"] = mod
...
class TestBuildRecord:
    def test_full_record(self):
        ...
        assert record["source"] == "at-austrocontrol-registry"
...
class TestWriteToRedis:
    ...
    def test_source_field_in_written_record(self):
        """Every record written to Redis must contain source='at-austrocontrol-registry'."""
        item = _make_item()
        r = _make_redis_with_search(icao_hex="440123", registration="OE-ARG")
        write_to_redis([item], r, REDIS_TTL)
        set_call = r.pipeline.return_value.json.return_value.set.call_args
        ...
        assert written.get("source") == "at-austrocontrol-registry"
...
class TestPublishCompletionStats:
    ...
    def test_mqtt_connect_timeout_does_not_raise(self):
        ...
        with patch("at_austrocontrol_registry_main.mqtt.Client") as mock_cls:
            mock_client = MagicMock()
    ...
    def test_mqtt_publishes_records_imported(self):
        ...
        with patch("at_austrocontrol_registry_main.mqtt.Client", return_value=mc):
    ...
    def test_mqtt_root_topic(self):
        assert MQTT_ROOT == "SkyFollower/runner/at-austrocontrol-registry"
...
class TestNetwork:
    ...
    def test_url_reachable(self):
        assert_url_reachable(_mod.API_URL, "at-austrocontrol-registry", headers={"User-Agent": "P5Software SkyFollower"})
