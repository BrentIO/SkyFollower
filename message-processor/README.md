...
- **`position`** -- sent at most once per `MAP_UDP_MIN_POSITION_INTERVAL_SECONDS`
  ...
  `timestamp`, `latitude`, `longitude`, `altitude`, `velocity`, `heading`,
  `vertical_speed`); a field absent from that message is omitted, not sent
  as null, matching `Position.to_dict()`/`Velocity.to_dict()`'s existing
  convention.
