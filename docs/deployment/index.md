...
| `docker-compose.map.yaml` | Live real-time aircraft map (own standalone host, given the expected UDP-ingest + WebSocket-fan-out load) | `map`, `map-redis` (dedicated, no-persistence Redis instance — never core Redis) |
...
| `map` | Backend for the live real-time aircraft map: UDP listener, REST snapshot, WebSocket relay | UDP `MAP_LISTEN_PORT`, HTTPS `MAP_HTTP_PORT` (default 443, self-signed by default; plain HTTP if no cert is present). Single port — no plain-HTTP redirect listener |
...
defaults the prompt to `map-redis` (the bundled container's service name)
rather than reading a `REDIS_HOST` value from another role's `.env`.
`MAP_REDIS_PASSWORD` is optional: `map-redis` ships with no auth by
...
[Map](https://github.com/BrentIO/SkyFollower/blob/main/map/README.md#frontend-configuration)'s
"Frontend Configuration" section. It's a Vite `import.meta.env.VITE_*`
...
--upgrade` re-resolves the latest release tag, re-fetches each role
directory's `docker-compose.*.yaml` and any `config/*.example` templates
(so a service, label, or port mapping added to a compose file since the
last install/upgrade actually reaches an existing deployment), rewrites
`SKYFOLLOWER_VERSION` in every role directory found under the install
root, and runs `docker compose pull && up -d` in each — no prompting. The
compose re-fetch never clobbers `docker-compose.message-processor.yaml` or
`docker-compose.receiver.yaml` (they hold per-instance service blocks
generated by earlier prompts) or a `config/*` file already derived from
its `.example` template — same no-clobber behavior a first install uses.
...
Build](/architecture/local-development#testing-a-dev-build).
