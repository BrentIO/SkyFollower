...
The frontend's own build-time configuration (`VITE_MAP_API_BASE_URL`,
`VITE_HOME_LATITUDE`/`VITE_HOME_LONGITUDE`) is not part of this table --
see [Map](https://github.com/BrentIO/SkyFollower/blob/main/map/README.md#frontend-configuration)'s
"Frontend Configuration" section. Those are Vite `import.meta.env.VITE_*`
values baked into the frontend's static bundle at `npm run build` time,
