...
| `MAX_MESSAGE_LAG_SECONDS` | 30 s | Maximum age of a source message before a downstream emission based on it is suppressed — e.g. an older rule match is still recorded in `matched_rules`, just not announced over MQTT, since it would no longer be actionable. The same guard applies to the message processor's optional map UDP `position`/`metadata` feed. |
