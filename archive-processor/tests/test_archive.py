...
# ---------------------------------------------------------------------------
# _DepthHWM — high-water-mark tracker backing rabbitmq_archive_queue_depth_hwm
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ArchiveProcessor._rmq_queue_depth — passive queue_declare on the archive
# queue, reusing the existing consumer channel
# ---------------------------------------------------------------------------
...
class TestRmqQueueDepth:
    def test_no_channel_returns_negative_one(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            processor._rmq_channel = None
            assert processor._rmq_queue_depth() == -1
    ...
    def test_returns_message_count_from_passive_declare(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            processor._rmq_channel = mock_channel

            assert processor._rmq_queue_depth() == 7
...
# ---------------------------------------------------------------------------
# ArchiveProcessor._rmq_queue_depth_sampler_loop — polls at most once every
# 10 seconds, independent of telemetry_interval_seconds
# ---------------------------------------------------------------------------

class TestRmqQueueDepthSamplerLoop:
    def _run_one_sample_tick(self, processor) -> None:
        """Run the real _rmq_queue_depth_sampler_loop for exactly one
        iteration, by making the mocked time.sleep set _shutdown so the
        loop body runs once and then exits — mirrors the message
        processor's _run_one_telemetry_tick test helper."""
        def fake_sleep(_seconds):
            processor._shutdown.set()

        with patch("archive_processor.main.time.sleep", side_effect=fake_sleep):
            processor._rmq_queue_depth_sampler_loop()
    ...
    def test_sleeps_ten_seconds_between_samples(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with patch("archive_processor.main.time.sleep") as mock_sleep:
                mock_sleep.side_effect = lambda _s: processor._shutdown.set()
                processor._rmq_queue_depth_sampler_loop()
    ...
    def test_one_tick_records_sampled_depth_into_hwm(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self._run_one_sample_tick(processor)

            assert processor._rmq_queue_depth_hwm.value_and_reset() == 12
    ...
    def test_one_tick_with_no_channel_records_negative_one(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert processor._rmq_queue_depth_hwm.value_and_reset() == -1


# ---------------------------------------------------------------------------
# ArchiveProcessor._publish_telemetry — rabbitmq_archive_queue_depth_hwm
# ---------------------------------------------------------------------------
...
class TestPublishTelemetryRmqQueueDepthHwm:
    ...
    def test_publishes_recorded_max_then_resets(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            processor._mqtt = mock_mqtt
            processor._mqtt_connected = True
            processor._rmq_queue_depth_hwm.record(3)
            processor._rmq_queue_depth_hwm.record(15)
